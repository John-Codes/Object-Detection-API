...
ok(!$invalid_response->is_success, "Invalid endpoint returns 404");
print "Status: " . $invalid_response->code . "\n";

# Test 5: Test malformed request
print "\n5. Testing malformed request...\n";
my $malformed_response = test_malformed_request();
ok(!$malformed_response->is_success, "Malformed request fails");
print "Status: " . $malformed_response->code . "\n";
...
sub handle_mock_request {
    ...
    if ($request->method eq 'POST' && $path eq '/detect') {
        ...
        $client->send_body($response_data);
...
sub test_invalid_endpoint {
    ...
    my $request = HTTP::Request->new(
        'GET',
        "http://localhost:$TEST_PORT/invalid"
    );
    return $ua->request($request);
}
